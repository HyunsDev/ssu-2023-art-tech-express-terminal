...
class WorkComponent(Element):
    def __init__(self, work, order):
        ...
        self.imageArea = Area(getX(order) + 10, 250 + 10, WORK_SIZE - 20, WORK_SIZE - 20)
    ...
    def __mouseOverHandler(self, event):
        if self.isFoucs: return
        self.backgroundArea.transition((self.x - 10, self.y - 10, WORK_SIZE + 20, WORK_SIZE + 20), 12, (0, 1, 0, 1))

    def __mouseOutHandler(self, event):
        if self.isFoucs: return
        self.backgroundArea.transition((self.x, self.y, WORK_SIZE, WORK_SIZE), 12, (0, 1, 0, 1))
    ...
    def __focus(self):
        ...
        imageTarget = (p5Context.width / 2 - WORK_SIZE / 2, p5Context.height / 2 - WORK_SIZE / 2, WORK_SIZE, WORK_SIZE)
        self.imageArea.transition(imageTarget, 12, (0, 1, 0, 1))
    ...
    def __unfocus(self):
        ...
        self.backgroundArea.transition((getX(self.order), 250, WORK_SIZE, WORK_SIZE), 12, (0, 1, 0, 1))
        self.imageArea.transition((getX(self.order) + 10, 250 + 10, WORK_SIZE - 20, WORK_SIZE - 20), 12, (0, 1, 0, 1))
